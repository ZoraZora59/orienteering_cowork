...
public abstract class AbstractCommandHandler implements ICommandHandler {
    ...
    public ResponseMessage runCommand(ReceiveMessage message) {
        try {
            ...
            ResponseMessage msg = handleCommand(message);
            log.info("执行命令成功: {}，耗时: {}ms", respondCommand(), System.currentTimeMillis() - start);
            return msg;
        } catch (Exception e) {
            log.error("执行命令失败", e);
            ResponseMessage msg = new ResponseMessage();
            msg.setContent("抱歉，执行失败，请稍后再试.(或者直接留言告诉我你遇到的问题，我会尽快解决，非常感谢！🥹");
    ...
    public abstract ResponseMessage handleCommand(ReceiveMessage message);
